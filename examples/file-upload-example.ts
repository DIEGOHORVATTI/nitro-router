...
const fileRouter = new NitroRouter()
  .use(authMiddleware)
  .group({ 
    prefix: '/files', 
    tags: ['Files'] 
...
fileRouter.get('/', async (ctx, res) => {
  ...
  let files = [
    { id: 1, name: 'documento.pdf', folder: 'documents', mimetype: 'application/pdf', owner: user.email },
    { id: 2, name: 'foto.jpg', folder: 'images', mimetype: 'image/jpeg', owner: user.email },
    { id: 3, name: 'planilha.xlsx', folder: 'documents', mimetype: 'application/vnd.ms-excel', owner: user.email }
  ]
  ...
  if (type !== 'all') {
    files = files.filter(f => {
      ...
      if (type === 'document') return f.mimetype.includes('pdf') || f.mimetype.includes('excel') || f.mimetype.includes('word')
      return true
    })
  }
...
fileRouter.get('/:id', async (ctx, res) => {
...
fileRouter.put('/:id', async (ctx, res) => {
...
fileRouter.delete('/:id', async (ctx, res) => {
  const { params, user } = ctx
  const fileId = parseInt(params.id)
...
fileRouter.post('/:id/share', async (ctx, res) => {
  const { params, body, user } = ctx
  const fileId = parseInt(params.id)
  
  console.log(`🤝 Usuário ${user.email} compartilhando arquivo ${fileId} com ${body.targetUserEmail}`)
...
fileRouter.delete('/:id/share/:targetUserId', async (ctx, res) => {
  const { params, user } = ctx
  const fileId = parseInt(params.id)
